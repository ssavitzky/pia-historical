...
public class RHProfile extends Component {
    RHProfileEditor parent;
    ...
    private int sensitivitySetting=0, numMeterStyles=0, locationFontSize=10, modeFontSize=10, motifNumber=0,
    ...
    private RH_LocationItem[] locationList;
    public RHActiveConcept[] allConcepts;
    public int numAllConcepts=0;
    ...
    public RHProfile (RHProfileEditor par) {
	parent=par;
  ...
  private boolean readConceptsContents(String conceptsString) {
    ...
    int i=0, j=0, k=0, x=0, num=0, numStyles=0, wordsProcessed=0, maxKeywords=RH_GlobalVars.maxNumberKeywords, maxWordStrings=10,numKeywords=0, active=0;
    Double prior;
    byte[] buffer, wordBuf=null; 
	...
	numAllConcepts=num;
	allConcepts=new RHActiveConcept[numAllConcepts];
    } catch (IOException ex) { success=false; System.out.println("Failed to read the next line"); }
	...
	System.out.println("...Saving user profile: "+path);
	File file=new File(path,profileFilename), bakFile=new File(path,profileFilename), 
	    bakbakFile=new File(path,profileFilename+bakExtension);
	...
	if (makeBackupFiles) {
	    ...
	    } catch (SecurityException ex) { new RH_PopupError(parent,"You cannot make a backup of this file - permission denied"); }
	    try {
		bakFile.renameTo(new File("./",profileFilename+bakExtension));
	    } catch (SecurityException ex) { new RH_PopupError(parent,"Could not rename back up file:"+profileFilename+bakExtension+
							       " - permission denied"); }
	}
	try {
	    ...
	    new RH_PopupError(parent,"Could not create profile file:"+file.toString()+" - permission denied"); 
	}
	return success;
    }
    
    public boolean saveConcepts(String path) {
	File file=new File(path,conceptsFilename), bakFile=new File(path,conceptsFilename), 
	    bakbakFile=new File(path,conceptsFilename+bakExtension);
	StringBuffer buffer=null;
	String newline=getNewlineByte();
	String[] concepts=null, topics=null;
	boolean success=false;
	...
	if (makeBackupFiles) {
	    ...
	    } catch (SecurityException ex) { new RH_PopupError(parent,"Could not rename back up file:"+conceptsFilename+bakExtension+
							       " - permission denied"); }
	}
	try {
	    FileOutputStream fp=new FileOutputStream(file.toString());   
	    ...
	    fp.write(buffer.toString().getBytes());
	    for (i=0;i<len;i++) {
		topics=allConcepts[i].getTopicKeywordStrings();
		if (allConcepts[i].isActive()) active=1;
		else active=0;
		buffer=new StringBuffer(allConcepts[i].getName()).append(newline).
		    append(allConcepts[i].getShortName()).append(newline).
		    append(allConcepts[i].getPrior()).append(newline).
	    ...
	    new RH_PopupError(parent,"Could not create concepts file:"+file.toString()+" - permission denied"); 
	}
	return success;
    }

    public boolean saveGroups(String path) {
	File file=new File(path,groupsFilename), bakFile=new File(path,groupsFilename), bakbakFile=new File(path,groupsFilename+bakExtension);
	StringBuffer buffer=null;
	String newline=getNewlineByte();
	...
	if (makeBackupFiles) {
	    ...
	    } catch (SecurityException ex) { new RH_PopupError(parent,"Could not rename back up file:"+groupsFilename+bakExtension+
							   " - permission denied"); }
	}
	try {
	    FileOutputStream fp=new FileOutputStream(file.toString());   
	    ...
	    new RH_PopupError(parent,"Could not create groups file:"+file.toString()+" - permission denied"); 
	}
	return success;
    }
  ...
  private boolean readLocationsContents(String locationsString) {
    ...
    locationList=new RH_LocationItem[numURLS];
	...
	visited=new Long(lastVisited);
      ...
      if (success) {
	locationList[processed++]=new RH_LocationItem(urlString,titleString,visited.longValue());
      }
    }
    return success;
  }
  ...
  public boolean writeLocationList() {
    ...
    urlStr=new String(""+locationList.length);
    // If everythiing is OK, continue ...
    if (success) {
      try {
	...
	for (int i=0;i<locationList.length;i++) {
	  long visited=locationList[i].getLastVisited();
	  String tmp=new String(""+visited);
	  dataOutput.write(locationList[i].getURL(),0,locationList[i].getURL().length());
	  dataOutput.newLine();
	  dataOutput.write(locationList[i].getTitle(),0,locationList[i].getTitle().length());
	  dataOutput.newLine();
